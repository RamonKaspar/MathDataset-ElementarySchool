...
def translate_json_to_german(input_path, output_path):
    df = pd.read_csv(input_path, dtype={'question': str})   # Ensure that the question column is read as string
...
def csv_to_json(input_path, output_path):
    df = pd.read_csv(input_path, dtype={'question': str, 'answer': float})
